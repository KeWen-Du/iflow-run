...
**新功能**
- 🎛️ **API Key 来源选择器** - 支持从环境变量或自定义配置获取 API Key，界面显示对应的环境变量名提示
- 📝 **提问改进建议** - AI 分析报告新增"提问改进建议"，针对用户提问提供优化建议
- 🤖 **AI 模型列表更新** - 支持心流开放平台所有模型，包括视觉模型、Kimi、Qwen、DeepSeek 等

**Bug 修复**
- 🔧 **主题设置同步修复** - 修复主题切换按钮和设置面板之间状态不同步的问题
- 🔧 **API 连接测试修复** - 支持使用环境变量 API Key 进行连接测试
- 🔧 **API 响应验证增强** - 增加 HTTP 200 响应体的错误检测，提高测试准确性
- 🔧 **异步函数调用修复** - 修复设置面板打开时 AI 配置可能未加载完成的问题

**重构优化**
- ♻️ **AI 配置简化** - 移除预设按钮，简化配置界面，环境变量方式成为默认推荐
...
**Bug 修复**
- 🔧 **打开 iflow 路径问题修复** - 修复工作目录包含特殊字符（如空格、单引号等）时无法正确打开的问题
- 🔧 **Windows PowerShell 路径转义** - 使用 `-LiteralPath` 参数和单引号转义，正确处理特殊路径
